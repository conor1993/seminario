...
namespace ORMDiocesis.DaoClasses
{
	...
	/// <summary>
	/// General DAO class for the Autor Entity. It will perform database oriented actions for
	/// a entity of type 'AutorEntity'. This DAO works on an EntityFields object. 
	/// </summary>
	public partial class AutorDAO : DaoBase
	{
		/// <summary>CTor</summary>
		public AutorDAO() : base(InheritanceInfoProviderSingleton.GetInstance(), new DynamicQueryEngine(), InheritanceHierarchyType.None, "AutorEntity", new AutorEntityFactory())
		{
		}
		...
		internal AutorDAO(IInheritanceInfoProvider inheritanceInfoProviderToUse, DynamicQueryEngineBase dqeToUse, InheritanceHierarchyType typeOfInheritance, string entityName, IEntityFactory entityFactory) : base(inheritanceInfoProviderToUse, dqeToUse, typeOfInheritance, entityName, entityFactory)
		{
		}


		/// <summary>
		/// Retrieves in the calling AutorCollection object all AutorEntity objects which have data in common
		...
		public bool GetMulti(ITransaction containingTransaction, IEntityCollection collectionToFill, long maxNumberOfItemsToReturn, ISortExpression sortClauses, IEntityFactory entityFactoryToUse, IPredicateExpression filter, IEntity paisInstance, int pageNumber, int pageSize)
		{
			base.EntityFactoryToUse = entityFactoryToUse;
			IEntityFields fieldsToReturn = EntityFieldsFactory.CreateEntityFieldsObject(ORMDiocesis.EntityType.AutorEntity);
			IPredicateExpression selectFilter = CreateFilterUsingForeignKeys(paisInstance, fieldsToReturn);
			if(filter!=null)
			{
		...
		/// <summary>
		/// Retrieves entities of the type 'AutorEntity' in a datatable which match the specified filter. 
		...
		/// <summary>
		/// Retrieves in the calling AutorCollection object all AutorEntity objects
		...
		public bool GetMultiUsingBcatEstatusCollectionViaLibro(ITransaction containingTransaction, IEntityCollection collectionToFill, long maxNumberOfItemsToReturn, ISortExpression sortClauses, IEntityFactory entityFactoryToUse, IEntity libroEstatusInstance, int pageNumber, int pageSize)
		{
			...
			relations.Add(AutorEntity.Relations.LibroEntityUsingIdAutor, "Libro_");
		...
		/// <summary>
		/// Deletes from the persistent storage all 'Autor' entities which have data in common
		/// with the specified related Entities. If one is omitted, that entity is not used as a filter. 
		/// </summary>
		/// <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or null if not.</param>
		/// <param name="paisInstance">PaisEntity instance to use as a filter for the AutorEntity objects to delete</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int DeleteMulti(ITransaction containingTransaction, IEntity paisInstance)
		{
			IEntityFields fields = EntityFieldsFactory.CreateEntityFieldsObject(ORMDiocesis.EntityType.AutorEntity);
			IPredicateExpression deleteFilter = CreateFilterUsingForeignKeys(paisInstance, fields);
		...
		/// <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or null if not.</param>
		/// <param name="paisInstance">PaisEntity instance to use as a filter for the AutorEntity objects to update</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		public int UpdateMulti(IEntity entityWithNewValues, ITransaction containingTransaction, IEntity paisInstance)
		{
			IEntityFields fields = EntityFieldsFactory.CreateEntityFieldsObject(ORMDiocesis.EntityType.AutorEntity);
			IPredicateExpression updateFilter = CreateFilterUsingForeignKeys(paisInstance, fields);
		...
		/// </summary>
		/// <param name="paisInstance">PaisEntity instance to use as a filter for the AutorEntity objects</param>
		/// <param name="fieldsToReturn">IEntityFields implementation which forms the definition of the fieldset of the target entity.</param>
		/// <returns>A ready to use PredicateExpression based on the passed in foreign key value holders.</returns>
		private IPredicateExpression CreateFilterUsingForeignKeys(IEntity paisInstance, IEntityFields fieldsToReturn)
		{
			IPredicateExpression selectFilter = new PredicateExpression();
			
			if(paisInstance != null)
			{
				selectFilter.Add(new FieldCompareValuePredicate(fieldsToReturn[(int)AutorFieldIndex.IdPais], ComparisonOperator.Equal, ((PaisEntity)paisInstance).Id));
			}
			return selectFilter;
		}
