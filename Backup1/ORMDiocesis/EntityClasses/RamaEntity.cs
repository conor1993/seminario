...
namespace ORMDiocesis.EntityClasses
{
			...
			
	{
		...
		private ORMDiocesis.CollectionClasses.TematicaCollection	_tematicas;
		private bool	_alwaysFetchTematicas, _alreadyFetchedTematicas;
		...
		{
			_tematicas = (ORMDiocesis.CollectionClasses.TematicaCollection)info.GetValue("_tematicas", typeof(ORMDiocesis.CollectionClasses.TematicaCollection));
			_alwaysFetchTematicas = info.GetBoolean("_alwaysFetchTematicas");
			_alreadyFetchedTematicas = info.GetBoolean("_alreadyFetchedTematicas");
		...
		{
			_alreadyFetchedTematicas = (_tematicas.Count > 0);
		...
		{
			info.AddValue("_tematicas", _tematicas);
			info.AddValue("_alwaysFetchTematicas", _alwaysFetchTematicas);
			info.AddValue("_alreadyFetchedTematicas", _alreadyFetchedTematicas);
		...
		public override void SetRelatedEntityProperty(string propertyName, IEntity entity)
		{
			switch(propertyName)
			{
				case "Seccion":
					...
					break;
				case "Tematicas":
					_alreadyFetchedTematicas = true;
					if(entity!=null)
					{
						this.Tematicas.Add((TematicaEntity)entity);
					}
					break;
		...
		public override void SetRelatedEntity(IEntity relatedEntity, string fieldName)
		{
			switch(fieldName)
			{
				case "Seccion":
					...
					break;
				case "Tematicas":
					_tematicas.Add((TematicaEntity)relatedEntity);
					OnRelatedEntitySet(relatedEntity, fieldName);
					break;
		...
		public override void UnsetRelatedEntity(IEntity relatedEntity, string fieldName, bool signalRelatedEntityManyToOne)
		{
			switch(fieldName)
			{
				case "Seccion":
					...
					break;
				case "Tematicas":
					base.PerformRelatedEntityRemoval(_tematicas, relatedEntity, signalRelatedEntityManyToOne);
					OnRelatedEntityUnset(relatedEntity, fieldName);
					break;
		...
		/// <summary> Retrieves all related entities of type 'TematicaEntity' using a relation of type '1:n'.</summary>
		/// <param name="forceFetch">if true, it will discard any changes currently in the collection and will rerun the complete query instead</param>
		/// <returns>Filled collection with all related entities of type 'TematicaEntity'</returns>
		public ORMDiocesis.CollectionClasses.TematicaCollection GetMultiTematicas(bool forceFetch)
		{
			return GetMultiTematicas(forceFetch, _tematicas.EntityFactoryToUse, null);
		}
		...
		/// <returns>Filled collection with all related entities of type 'TematicaEntity'</returns>
		public ORMDiocesis.CollectionClasses.TematicaCollection GetMultiTematicas(bool forceFetch, IPredicateExpression filter)
		{
			return GetMultiTematicas(forceFetch, _tematicas.EntityFactoryToUse, filter);
		}
		...
		public ORMDiocesis.CollectionClasses.TematicaCollection GetMultiTematicas(bool forceFetch, IEntityFactory entityFactoryToUse)
		{
			return GetMultiTematicas(forceFetch, entityFactoryToUse, null);
		}
		...
		/// <param name="filter">Extra filter to limit the resultset.</param>
		/// <returns>Filled collection with all related entities of the type constructed by the passed in entity factory</returns>
		public virtual ORMDiocesis.CollectionClasses.TematicaCollection GetMultiTematicas(bool forceFetch, IEntityFactory entityFactoryToUse, IPredicateExpression filter)
		{
 			if( ( !_alreadyFetchedTematicas || forceFetch || _alwaysFetchTematicas) && !base.IsSerializing && !base.IsDeserializing && !base.InDesignMode)
			{
				if(base.ParticipatesInTransaction)
				{
					if(!_tematicas.ParticipatesInTransaction)
					{
						base.Transaction.Add(_tematicas);
					}
				}
				_tematicas.SuppressClearInGetMulti=!forceFetch;
				if(entityFactoryToUse!=null)
				{
					_tematicas.EntityFactoryToUse = entityFactoryToUse;
				}
				_tematicas.GetMultiManyToOne(this, null, filter);
				_tematicas.SuppressClearInGetMulti=false;
				_alreadyFetchedTematicas = true;
			}
			return _tematicas;
		}

		/// <summary> Sets the collection parameters for the collection for 'Tematicas'. These settings will be taken into account
		/// when the property Tematicas is requested or GetMultiTematicas is called.</summary>
		/// <param name="maxNumberOfItemsToReturn"> The maximum number of items to return. When set to 0, this parameter is ignored</param>
		/// <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified (null), no sorting is applied.</param>
		public virtual void SetCollectionParametersTematicas(long maxNumberOfItemsToReturn, ISortExpression sortClauses)
		{
			_tematicas.SortClauses=sortClauses;
			_tematicas.MaxNumberOfItemsToReturn=maxNumberOfItemsToReturn;
		}
		...
		public virtual SeccionEntity GetSingleSeccion(bool forceFetch)
		{
			if( ( !_alreadyFetchedSeccion || forceFetch || _alwaysFetchSeccion) && !base.IsSerializing && !base.IsDeserializing )			
			{
				...
				if(base.CheckIfLazyLoadingShouldOccur(RamaEntity.Relations.SeccionEntityUsingIdSeccion))
				{
		...
		{
			_tematicas.ActiveContext = base.ActiveContext;
		...
		{
			_tematicas = new ORMDiocesis.CollectionClasses.TematicaCollection(new TematicaEntityFactory());
			_tematicas.SetContainingEntityInfo(this, "Rama");
			_alwaysFetchTematicas = false;
			_alreadyFetchedTematicas = false;
		...
		{
			base.PerformDesetupSyncRelatedEntity( _seccion, new PropertyChangedEventHandler( OnSeccionPropertyChanged ), "Seccion", RamaEntity.Relations.SeccionEntityUsingIdSeccion, true, signalRelatedEntity, "Ramas", resetFKFields, new int[] { (int)RamaFieldIndex.IdSeccion } );		
			_seccion = null;
		}
		...
		private void SetupSyncSeccion(IEntity relatedEntity)
		{
			...
			base.PerformSetupSyncRelatedEntity( _seccion, new PropertyChangedEventHandler( OnSeccionPropertyChanged ), "Seccion", RamaEntity.Relations.SeccionEntityUsingIdSeccion, true, ref _alreadyFetchedSeccion, new string[] {  } );
		}
		...
		public static IPrefetchPathElement PrefetchPathTematicas
		{
			get
			{
				return new PrefetchPathElement(new ORMDiocesis.CollectionClasses.TematicaCollection(),
					RamaEntity.Relations.TematicaEntityUsingIdRama, 
					(int)ORMDiocesis.EntityType.RamaEntity, (int)ORMDiocesis.EntityType.TematicaEntity, 0, null, null, null, "Tematicas", SD.LLBLGen.Pro.ORMSupportClasses.RelationType.OneToMany);
			}
		}
		...
		public static IPrefetchPathElement PrefetchPathSeccion
		{
			get
			{
				return new PrefetchPathElement(new ORMDiocesis.CollectionClasses.SeccionCollection(),
					RamaEntity.Relations.SeccionEntityUsingIdSeccion, 
					(int)ORMDiocesis.EntityType.RamaEntity, (int)ORMDiocesis.EntityType.SeccionEntity, 0, null, null, null, "Seccion", SD.LLBLGen.Pro.ORMSupportClasses.RelationType.ManyToOne);
			}
		}
		...
		/// <summary> The IdSeccion property of the Entity Rama<br/><br/>
		/// </summary>
		/// <remarks>Mapped on  table field: "BCatRamas"."IdSeccion"<br/>
		...
		public virtual System.Int32 IdSeccion
		{
			get
			{
				object valueToReturn = base.GetCurrentFieldValue((int)RamaFieldIndex.IdSeccion);
			...
			set	{ SetNewFieldValue((int)RamaFieldIndex.IdSeccion, value); }
		}
		...
		/// <summary> Retrieves all related entities of type 'TematicaEntity' using a relation of type '1:n'.</summary>
		/// <remarks>This property is added for databinding conveniance, however it is recommeded to use the method 'GetMultiTematicas()', because 
		/// this property is rather expensive and a method tells the user to cache the result when it has to be used more than once in the same scope.</remarks>
		public virtual ORMDiocesis.CollectionClasses.TematicaCollection Tematicas
		{
			get	{ return GetMultiTematicas(false); }
		}

		/// <summary> Gets / sets the lazy loading flag for Tematicas. When set to true, Tematicas is always refetched from the 
		/// persistent storage. When set to false, the data is only fetched the first time Tematicas is accessed. You can always execute
		/// a forced fetch by calling GetMultiTematicas(true).</summary>
		[Browsable(false)]
		public bool AlwaysFetchTematicas
		{
			get	{ return _alwaysFetchTematicas; }
			set	{ _alwaysFetchTematicas = value; }	
		}
			...
			set
			{
				...
				else
				{
					if(value==null)
					{
						if(_seccion != null)
						{
							_seccion.UnsetRelatedEntity(this, "Ramas");
						}
					}
					else
					{
						((IEntity)value).SetRelatedEntity(this, "Ramas");
					}
				}
			}
		}
