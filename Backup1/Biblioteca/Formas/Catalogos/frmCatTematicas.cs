    ...
    {        
        ...
        void CN.iForm.Guardar()
        {
            ...
            try
            {
                ...
                loTematica.IdSeccion = (int)cmbSeccion.SelectedValue;
                loTematica.IdRama = (int)cmbRama.SelectedValue;
        ...
        private bool Validar()
        {
            ...
            if (cmbSeccion.SelectedIndex==-1)
                Mensaje = Mensaje + "   * No se ha capturado la sección.\n";
        ...
        void CN.iForm.Borrar()
        {
            ...
            if (loTematica.Libros.Count > 0)
            {
                MessageBox.Show("No se puede borrar la temática porque tiene libros capturados", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            ...
            try
            {
                if (DataGrid.SelectedRows.Count > 0)
                {
                    ...
                    if (loTematica.Obtener(Convert.ToInt32(DataGrid.SelectedRows[0].Cells[0].Value)))
                    {
                        ...
                        cmbSeccion.SelectedValue = loTematica.IdSeccion;
                        cmbRama.SelectedValue = loTematica.IdRama;
        ...
        private void cmbSeccion_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cmbSeccion.SelectedIndex == -1)
            {
                cmbRama.DataSource = null;
            }
            else
            {
                OC.PredicateExpression filtro = new OC.PredicateExpression();
                filtro.Add(HC.RamaFields.IdSeccion == (int)cmbSeccion.SelectedValue);
                loColRamas.Obtener(filtro);
                cmbRama.DisplayMember = "Nombre";
                cmbRama.ValueMember = "Id";
                cmbRama.DataSource = loColRamas;
                cmbRama.SelectedIndex = -1;
